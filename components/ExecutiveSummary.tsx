...
import { ExecutiveSummarySkeleton } from '@/components/AnalysisSkeleton';
import { compositeRiskScore as fallbackCompositeRiskScore } from '@/config/risk-data';
import { situationData as fallbackSituationData } from '@/config/situation-data';
import { scenarios as fallbackScenarios } from '@/config/scenarios';
...
export default function ExecutiveSummary() {
  ...
  const riskScore = data?.compositeRiskScore ?? fallbackCompositeRiskScore;
  const sitData = data?.situationData ?? fallbackSituationData;
  ...
  const fetchDeltas = useCallback(async () => {
    try {
      ...
      const built: DeltaItem[] = [
        {
          label: 'Brent Crude',
          value: brent?.changePercent != null ? `${brent.changePercent >= 0 ? '+' : ''}${brent.changePercent.toFixed(1)}%` : 'N/A',
          direction: brent?.changePercent != null ? (brent.changePercent > 0 ? 'up' : 'down') : 'neutral',
        },
        { label: 'Freight Est.', value: freightImpact, direction: 'up' },
        {
          label: 'THB / USD',
          value: thb?.changePercent != null ? `${thb.changePercent >= 0 ? '+' : ''}${thb.changePercent.toFixed(2)}%` : 'N/A',
          direction: thb?.changePercent != null ? (thb.changePercent > 0 ? 'up' : 'down') : 'neutral',
        },
      ];
      setDeltas(built);
    } catch {
  ...
  if (loading && !data) return <ExecutiveSummarySkeleton />;

  const threatTextColor: Record<string, string> = {
    CRITICAL: 'text-red-400', HIGH: 'text-orange-400', ELEVATED: 'text-yellow-400',
    MODERATE: 'text-blue-400', LOW: 'text-green-400',
  };
  const threatGlow: Record<string, string> = {
    CRITICAL: 'glow-red', HIGH: 'glow-amber', ELEVATED: 'glow-amber',
    MODERATE: 'glow-cyan', LOW: 'glow-green',
  };
  const threatColor = threatTextColor[sitData.threatLevel] ?? 'text-red-400';
  const threatGlowClass = threatGlow[sitData.threatLevel] ?? 'glow-red';
      ...
      <div className="flex items-center gap-6">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-3 mb-3">
            <span className="type-label" style={{ letterSpacing: '0.2em' }}>Executive Summary</span>
            ...
            <span className={`font-mono text-[10px] font-bold tracking-[0.2em] uppercase ${threatColor} ${threatGlowClass}`}>
              {sitData.operationName}
            </span>
          </div>
          <div className="space-y-2.5">
            <div className="flex gap-3 items-baseline">
              <span className={`type-label flex-shrink-0 w-[82px] ${threatColor} ${threatGlowClass}`} style={{ fontSize: '9px' }}>KEY CHANGE</span>
              <p className="text-[14px] font-medium text-gray-100 leading-snug type-body">{sitData.bluf.keyChange}</p>
            </div>
            <div className="flex gap-3 items-baseline">
              <span className="type-label flex-shrink-0 w-[82px] text-orange-400/80 glow-amber" style={{ fontSize: '9px' }}>IMPACT</span>
              <p className="text-[13px] text-gray-300 leading-snug type-body">{sitData.bluf.impact}</p>
            </div>
            <div className="flex gap-3 items-baseline">
              <span className="type-label flex-shrink-0 w-[82px] text-amber-400/75" style={{ fontSize: '9px' }}>WATCH</span>
              <p className="text-[13px] text-gray-400 leading-snug type-body">{sitData.bluf.watch}</p>
            </div>
          </div>
        </div>
            ...
            {deltas.map((d, idx) => (
              <div
                ...
                className={`flex flex-col items-center justify-center px-4 py-2.5 rounded-sm min-w-[95px] number-reveal ${
                  d.direction === 'up' ? 'glow-red-box' : d.direction === 'down' ? 'glow-green-box' : ''
                }`}
                style={{
                  animationDelay: `${0.4 + idx * 0.1}s`,
                  background: d.direction === 'up' ? 'rgba(239, 68, 68, 0.04)' : d.direction === 'down' ? 'rgba(34, 197, 94, 0.04)' : 'rgba(100, 116, 139, 0.04)',
                  border: `1px solid ${d.direction === 'up' ? 'rgba(239, 68, 68, 0.2)' : d.direction === 'down' ? 'rgba(34, 197, 94, 0.2)' : 'rgba(100, 116, 139, 0.15)'}`,
                }}
              >
                <span className="type-label mb-1.5" style={{ fontSize: '8px' }}>{d.label}</span>
                <div className="flex items-center gap-1">
                  ...
                  <span className={`text-lg font-bold font-mono tabular-nums leading-none ${
                    d.direction === 'up' ? 'text-red-400 glow-red' : d.direction === 'down' ? 'text-green-400 glow-green' : 'text-gray-400'
                  }`}>{d.value}</span>
                </div>
              </div>
            ))}
