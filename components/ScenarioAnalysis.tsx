      ...
      <div className="flex-1 p-4 grid grid-cols-3 gap-3 min-h-0">
        {scenarios.map((scenario) => {
          ...
          return (
            <div
              key={scenario.id}
              className={`rounded-sm border ${cfg.border} ${cfg.bg} flex flex-col overflow-hidden`}
            >
              {/* Card Header */}
              <div className={`${cfg.headerBg} border-b ${cfg.border} px-3 py-2`}>
                <div className="flex items-center justify-between mb-1.5">
                  <span className={`text-[11px] font-mono font-bold ${cfg.text} tracking-wider`}>
                    {scenario.name}
                  </span>
                  <span
                    className={`text-[10px] font-mono font-bold px-1.5 py-0.5 rounded border ${cfg.badge}`}
                  >
              ...
              {/* Description */}
              <div className="px-3 py-2.5 border-b border-[#1a2a3a]">
              ...
              <div className="px-3 py-1.5 border-b border-[#1a2a3a] flex items-center gap-2">
                ...
                <span className={`text-[10px] font-mono font-medium ${cfg.text}`}>
                  {scenario.timeline}
                </span>
              </div>
