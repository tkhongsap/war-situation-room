...
function SectionHeader({ title }: { title: string }) {
  return (
    <div className="flex items-center gap-3 mb-3">
      <h3 className="type-label whitespace-nowrap" style={{ fontSize: '10px' }}>{title}</h3>
      <div className="flex-1 h-px" style={{ background: 'linear-gradient(90deg, rgba(30, 55, 85, 0.4), transparent)' }}></div>
    </div>
  );
}
...
function SkeletonCard() {
  return (
    <div className="rounded-sm border border-[#1a2a3a]/50 bg-[#0a0e16] p-3 animate-pulse">
      <div className="h-3 bg-gray-800/50 rounded w-16 mb-2"></div>
      <div className="h-5 bg-gray-800/50 rounded w-24 mb-3"></div>
...
function SkeletonRow() {
  return (
    <div className="flex items-center justify-between py-2 border-b border-[#0f1825]/50 animate-pulse">
      <div className="h-3 bg-gray-800/50 rounded w-20"></div>
...
function CompactPriceRow({ item, decimals = 2 }: { item: PriceDataPoint; decimals?: number }) {
  ...
  return (
    <div className="flex items-center justify-between py-1.5 border-b border-[#0f1825]/60 last:border-0 hover:bg-[#0c1018] transition-colors group">
      <span className="text-[11px] text-gray-400 min-w-[90px] type-body">{item.name}</span>
    ...
    <div className="space-y-5">
      ...
      <div className="flex items-center justify-between mb-1">
        <div className="flex items-center gap-2.5">
          ...
          <h2 className="type-display text-[12px] text-gray-300">Market Signals</h2>
        </div>
        {lastUpdated && (
      ...
      <div>
        ...
        <div className="grid grid-cols-3 gap-3">
          {loading ? (
            ...
            energy.map((item, idx) => (
              <div key={item.symbol} className="fade-in" style={{ animationDelay: `${idx * 0.08}s` }}>
                <PriceCard {...item} />
              </div>
            ))
          )}
        ...
        <div className="grid grid-cols-3 md:grid-cols-6 gap-3">
          {loading ? (
            ...
            commodities.map((item, idx) => (
              <div key={item.symbol} className="fade-in" style={{ animationDelay: `${idx * 0.06}s` }}>
                <PriceCard {...item} />
              </div>
            ))
          )}
      ...
      <div className="grid grid-cols-2 gap-6">
        <div>
          ...
          <div className="rounded-sm px-3 py-1" style={{ background: 'rgba(10, 14, 22, 0.6)', border: '1px solid rgba(30, 55, 85, 0.3)' }}>
            {loading ? (
              <>{[1,2,3,4].map(i => <SkeletonRow key={i} />)}</>
            ) : currencies.length === 0 ? (
              <p className="text-[11px] text-gray-600 py-3 text-center font-mono">Data unavailable</p>
            ) : (
              currencies.map((item, idx) => (
                <div key={item.symbol} className="fade-in" style={{ animationDelay: `${idx * 0.05}s` }}>
